...
func (p *Pool) QueryRow(_ context.Context, _ string, _ ...any) db.Row {
	if len(p.rowQueue) == 0 {
		return &Row{err: fmt.Errorf("fake pool: no QueryRow response queued")}
	}
	r := p.rowQueue[0]
...
func (p *Pool) Query(_ context.Context, _ string, _ ...any) (pgx.Rows, error) {
	if len(p.rowsQueue) == 0 {
		return nil, fmt.Errorf("fake pool: no Query response queued")
	}
	r := p.rowsQueue[0]
...
func (r *Rows) Next() bool                                   { r.idx++; return r.idx <= len(r.data) }
func (r *Rows) Close()                                       {}
...
func assignAll(dests, vals []any) error {
	for i, dst := range dests {
		...
		if err := assignOne(dst, val); err != nil {
