...
func (s *State) MyPaths(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		zap.S().Errorw("failed to query path enrollments", "err", err)
	...
	for _, enr := range enrollments {
		...
		if err != nil {
			zap.S().Warnw("failed to fetch path detail", "slug", enr.slug, "err", err)
...
func (s *State) completedCoursesCtx(request *http.Request, userID string, slugs []string) map[string]bool {
	...
	rows, err := s.Pool.Query(request.Context(),
		...
		userID, slugs)
	if err != nil {
		zap.S().Errorw("failed to query completed courses", "userID", userID, "err", err)
...
func (s *State) AdminListPathEnrollments(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		zap.S().Warnw("failed to fetch path detail for enrollments", "slug", slug, "err", err)
	}
	...
	if err != nil {
		zap.S().Errorw("failed to query path enrollments", "slug", slug, "err", err)
		s.Error(writer, http.StatusInternalServerError, "Database error")
...
func (s *State) AdminEnrollUserInPath(writer http.ResponseWriter, request *http.Request) {
	...
	_, err = s.Pool.Exec(request.Context(),
		...
		body.UserID, slug)
	if err != nil {
		zap.S().Errorw("failed to enroll user in path", "slug", slug, "userID", body.UserID, "err", err)
		s.Error(writer, http.StatusInternalServerError, "Database error")
...
func (s *State) AdminUnenrollUserFromPath(writer http.ResponseWriter, request *http.Request) {
	...
	_, err := s.Pool.Exec(request.Context(),
		...
		userID, slug)
	if err != nil {
		zap.S().Errorw("failed to unenroll user from path", "slug", slug, "userID", userID, "err", err)
		s.Error(writer, http.StatusInternalServerError, "Database error")
