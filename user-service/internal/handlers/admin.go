...
func (s *State) ListUsers(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&u.ID, &u.Username, &u.Email, &u.Role, &u.IsActive,
			&u.AvatarURL, &u.Bio, &u.AuthProvider, &u.CreatedAt, &u.EnrolledCourses)
		if err != nil {
...
// @Produce   json
// @Param     user_id  path  string  true  "User UUID"
// @Param     body     object  true  "Fields: username, bio, avatar_url, is_active, role (all optional)"
...
// @Produce   json
// @Param     slug  path  string  true  "Course slug"
// @Param     body  object  true  "user_id (UUID)"
// @Success   200   {object}  map[string]string
...
// @Produce   json
// @Param     slug  path  string  true  "Course slug"
// @Param     body  object  true  "group_id (UUID)"
// @Success   200   {object}  map[string]interface{}
